...
function parseRutaSheet(shRuta) {
  ...
  for (let r = 10; r < 2000; r++) {
    ...
    if (!isFilled(rutaId)) break;

    const amieAfc = normUpper(getCell(shRuta, `C${r}`));
    const benTotal = toNumber(getCell(shRuta, `D${r}`));
    const ini = toNumber(getCell(shRuta, `F${r}`)) ?? 0;
    const egb = toNumber(getCell(shRuta, `G${r}`)) ?? 0;
    const bac = toNumber(getCell(shRuta, `H${r}`)) ?? 0;
    const distRuta = toNumber(getCell(shRuta, `K${r}`));
    const distLineal = getCell(shRuta, `L${r}`);
    const x = getCell(shRuta, `M${r}`);
    const y = getCell(shRuta, `N${r}`);
  ...
  const totalBenef = rows.reduce((s, it) => s + (it.benTotal || 0), 0);
  const inconsist = rows.filter((it) => {
    if (it.benTotal === null) return true;
    const sum = (it.ini || 0) + (it.egb || 0) + (it.bac || 0);
    return Math.abs(sum - it.benTotal) > 0.5;
  });

  const coordHeaderOk = toNumber(coordX) !== null && toNumber(coordY) !== null;

  const coordRowsMissing = rows.filter((it) => {
    // si hay #N/D o vacío en X/Y
    const nx = toNumber(it.x);
    const ny = toNumber(it.y);
    return nx === null || ny === null || hasND(it.x) || hasND(it.y);
  });

  const distLinealBad = rows.filter((it) => hasND(it.distLineal) || toNumber(it.distLineal) === null);

  return {
    distrito,
    amieEje,
    coordHeaderOk,
    coordHeader: { x: coordX, y: coordY },
    rows,
    totalBenef,
    inconsistCount: inconsist.length,
    coordRowsMissingCount: coordRowsMissing.length,
    distLinealBadCount: distLinealBad.length,
    afcList: [...new Set(rows.map((it) => it.amieAfc).filter((v) => v))],
  };
}
...
function parseHistorico(wbHist, amieEje) {
  ...
  for (let i = 0; i < Math.min(matrix.length, 60); i++) {
    const row = (matrix[i] || []).map((v) => normUpper(v));
    const a = row.findIndex((c) => c === "AMIE EJE" || c.includes("AMIE") && c.includes("EJE"));
    if (a >= 0) {
      headerRow = i;
      idxAmie = a;
      idxRutas = row.findIndex((c) => c.includes("RUTA") && (c.includes("TOTAL") || c.includes("NRO") || c === "RUTAS"));
      idxBen = row.findIndex((c) => c.includes("BEN") && (c.includes("TOTAL") || c.includes("POB") || c.includes("ESTU")));
      idxPresA = row.findIndex((c) => c.includes("PRES") && c.includes("A"));
      break;
    }
  }

  // if no header, still attempt fixed layout (col J only)
  const matches = [];
  for (let i = (headerRow >= 0 ? headerRow + 1 : 1); i < matrix.length; i++) {
    const row = matrix[i] || [];
    const a = normUpper(row[idxAmie]);
    if (!a) continue;
    if (a === normUpper(amieEje)) {
      const rutas = idxRutas >= 0 ? toNumber(row[idxRutas]) : null;
      const ben = idxBen >= 0 ? toNumber(row[idxBen]) : null;
      const presA = idxPresA >= 0 ? toNumber(row[idxPresA]) : null;
      matches.push({ i: i + 1, rutas, ben, presA });
    }
  }

  // agregados
  const rutas = matches.map((m) => m.rutas).filter((x) => x !== null);
  const ben = matches.map((m) => m.ben).filter((x) => x !== null);
  const presA = matches.map((m) => m.presA).filter((x) => x !== null);

  return {
    sheetName,
    matchCount: matches.length,
    rutas: rutas.length ? rutas[0] : null,
    ben: ben.length ? ben[0] : null,
    presA: presA.length ? presA[0] : null,
    matches,
  };
}
    ...
    .map((c) => {
      ...
      return `
        <div class="checkRow ${cls}">
          <div class="checkCode">${c.codigo}</div>
          <div class="checkBody">
            <div class="checkTitle">${c.descripcion}</div>
            <div class="checkDetail">${c.detalle || ""}</div>
          </div>
          <div class="checkStatus">${c.status}</div>
        </div>`;
    })
    .join("");

  const extraHtml = extras.length
    ? `<div class="extraBox"><h3>Comparativo / Notas</h3>${extras
        .map((t) => `<div class="extraLine">• ${t}</div>`)
        .join("")}</div>`
    : "";

  box.innerHTML = rowsHtml + extraHtml;
}
