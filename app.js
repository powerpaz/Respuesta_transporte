...
function findValueNearLabel(matrix, labelMatchers) {
  // Busca una celda cuyo texto cumpla TODOS los matchers (substrings, en mayúsculas)
  // y retorna el valor a la derecha (c+1..c+3) que no esté vacío.
  const wants = (labelMatchers || []).map(x => normUpper(x));
  for (let r = 0; r < matrix.length; r++) {
    const row = matrix[r] || [];
    for (let c = 0; c < row.length; c++) {
      const cell = normUpper(row[c]);
      if (!cell) continue;
      const ok = wants.every(w => cell.includes(w));
      if (!ok) continue;

      for (let k = 1; k <= 3; k++) {
        const v = row[c + k];
        if (isFilled(v)) return norm(v);
      }
    }
  }
  return "";
...
function extractZonaFromMemo(memoNro, memoText) {
  const src = `${memoNro || ""} ${memoText || ""}`;
  // 1) CZ5 / CZ-5 / -CZ5-
  let m = src.match(/\bCZ\s*-?\s*(\d{1,2})\b/i);
  if (m) return `Zona ${m[1]}`;
  // 2) "Zona 5" explícito
  m = src.match(/\bZONA\s*-?\s*(\d{1,2})\b/i);
  if (m) return `Zona ${m[1]}`;
  return "";
}
...
// Metadatos (cabecera): nombre de Distrito / nombre de IE eje (heurístico por rótulos)
const nombreDistrito = findValueNearLabel(matrix, ["NOMBRE", "DISTRITO"]);
// En los formatos suele aparecer como "UNIDAD EDUCATIVA" o "NOMBRE UNIDAD EDUCATIVA"
const nombreIE = findValueNearLabel(matrix, ["UNIDAD", "EDUCATIVA"]) || findValueNearLabel(matrix, ["NOMBRE", "UNIDAD"]);
...
function parseHistorico(wbHist, amieEje) {
  ...
  const out = { sheetName, matchCount: 0, rutas: null, ben: null, presAnual: null, matches: [] };
  ...
  // En base histórica 2025 viene como "Prespuesto anual" (typo) o similar
  const idxPresA = header.findIndex((x) => x === "PRESPUESTO ANUAL" || x === "PRESUPUESTO ANUAL" || x === "PRESUPUESTO" || x === "PRESUPUESTO ANUAL (USD)");
  ...
  for (let r = headerIdx + 1; r < matrix.length; r++) {
    ...
    const p = idxPresA >= 0 ? toNumber(row[idxPresA]) : null;
    if (p !== null) presASum += p;

    out.matches.push({ row: r + 1, afc, ben: b, presA: p });
  }
  ...
  out.presAnual = out.matchCount ? presASum : null;
  return out;
}

// INFORME: Presupuesto referencial (tabla "PRESUPUESTO REFERENCIAL")
function parseInformePresupuesto(wbModel) {
  const sh = wbModel.Sheets["INFORME"];
  if (!sh) return { error: true, motivo: "No existe hoja INFORME" };

  const matrix = XLSX.utils.sheet_to_json(sh, { header: 1, defval: "" });
  // Buscamos el rótulo "Costo total" y tomamos la celda a la derecha.
  let costoTotal = null;
  let costoUnit = null;
  let ben = null;
  let dias = null;

  for (let r = 0; r < Math.min(matrix.length, 260); r++) {
    const row = matrix[r] || [];
    for (let c = 0; c < Math.min(row.length, 60); c++) {
      ...
      const t = normUpper(v);
      if (t === "COSTO TOTAL") {
        costoTotal = toNumber(row[c + 1]);
      } else if (t === "COSTO UNITARIO") {
        costoUnit = toNumber(row[c + 1]);
      } else if (t === "NUMERO DE BENEFICIARIOS" || t === "NÚMERO DE BENEFICIARIOS") {
        ben = toNumber(row[c + 1]);
      } else if (t.includes("DIAS") && t.includes("LABOR")) {
        dias = toNumber(row[c + 1]);
      }
    }
  }
...
function renderChecks(checks, extras = []) {
  ...
  const rowHtml = (c) => {
    ...
    return `
      <div class="checkRow2 ${cls}" data-status="${c.status}">
        <div class="checkLeft">
  ...
  if (state?.modelData?.ids?.nombreDistrito) hi.push({ k: "Nombre distrito", v: state.modelData.ids.nombreDistrito });
  if (state?.modelData?.ids?.zona) hi.push({ k: "Zona", v: state.modelData.ids.zona });
  if (state?.modelData?.ids?.amieEje) hi.push({ k: "AMIE Eje", v: state.modelData.ids.amieEje });
  if (state?.modelData?.ids?.nombreIE) hi.push({ k: "IE Eje", v: state.modelData.ids.nombreIE });
  ...
  if (state?.modelData?.informe?.costoTotal != null) hi.push({ k: "Presupuesto (INFORME)", v: formatMoney(state.modelData.informe.costoTotal) });
  ...
  if (state?.compareData && state?.modelData?.rutas?.total != null) {
    ...
    if (h.presAnual != null) hi.push({ k: "Histórico presupuesto", v: formatMoney(h.presAnual) });
    if (h.presAnual != null && state?.modelData?.informe?.costoTotal != null) {
      const delta = state.modelData.informe.costoTotal - h.presAnual;
      const pct = h.presAnual ? (delta / h.presAnual) * 100 : null;
      if (pct != null && isFinite(pct)) {
        hi.push({ k: "Δ Presupuesto", v: `${delta >= 0 ? "+" : ""}${formatMoney(delta)} • ${(pct >= 0 ? "+" : "")}${pct.toFixed(2)}%` });
      }
    }
  }
  ...
  const filterHtml = `
    <div class="filterBar" id="filterBar">
      <button class="filterBtn active" data-filter="ALL">TODOS (${checks.length})</button>
      <button class="filterBtn ok" data-filter="OK">OK (${counts.OK})</button>
      <button class="filterBtn warn" data-filter="WARN">WARN (${counts.WARN})</button>
      <button class="filterBtn fail" data-filter="FAIL">FAIL (${counts.FAIL})</button>
    </div>`;
  ...
  box.innerHTML = summaryHtml + filterHtml + highlightsHtml + groupsHtml + extrasHtml;
  setupFilterUI();
}
...
function setupFilterUI() {
  ...
  const buttons = bar.querySelectorAll(".filterBtn");
  buttons.forEach(btn => {
...
function applyFilter(filter) {
  const rows = document.querySelectorAll(".checkRow2");
  rows.forEach(r => {
  ...
  // hide accordions with no visible rows
  document.querySelectorAll("details.acc").forEach(acc => {
    const visible = Array.from(acc.querySelectorAll(".checkRow2")).some(r => r.style.display !== "none");
...
async function validateAll() {
  ...
  // INFORME – Presupuesto referencial (Costo total)
  const infPres = parseInformePresupuesto(wb);
  if (infPres.error) {
    checks.push({ codigo: "INF-06", descripcion: "INFORME: presupuesto referencial (Costo total)", status: "WARN", detalle: infPres.motivo || "No se pudo leer." });
  } else if (infPres.costoTotal === null || !(infPres.costoTotal > 0)) {
    checks.push({ codigo: "INF-06", descripcion: "INFORME: presupuesto referencial (Costo total)", status: "WARN", detalle: "No se encontró 'Costo total' o no es numérico." });
  } else {
    checks.push({ codigo: "INF-06", descripcion: "INFORME: presupuesto referencial (Costo total)", status: "OK", detalle: `Costo total: ${formatMoney(infPres.costoTotal)}` });
  }

  // 6) RUTA – cabecera, rutas, coords, distancias
  const rutaParsed = parseRutaSheet(shRuta);
  const zona = extractZonaFromMemo(state.memoFields.memo_nro, state.memoText);
  ...
  if (state.histXlsx && rutaParsed.amieEje) {
    ...
    if (hist.matchCount === 0) {
      ...
      // Presupuesto: comparamos histórico (Prespuesto anual) vs actual (INFORME: Costo total)
      if (hist.presAnual !== null && infPres && !infPres.error && infPres.costoTotal !== null) {
        const histP = hist.presAnual;
        const actP = infPres.costoTotal;
        const deltaP = actP - histP;
        const pct = histP ? (deltaP / histP) * 100 : null;
        const pctTxt = pct === null ? "" : ` • % ${(pct >= 0 ? "+" : "")}${pct.toFixed(2)}%`;
        compareExtras.push(`Presupuesto (INFORME vs histórico): histórico ${histP.toFixed(2)} vs actual ${actP.toFixed(2)} (Δ ${deltaP.toFixed(2)})${pctTxt}.`);

        const st = pct === null ? "WARN" : Math.abs(pct) <= 0.01 ? "OK" : "WARN";
        checks.push({
          codigo: "CMP-03",
          descripcion: "Comparación presupuesto vs año anterior (INFORME)",
          status: st,
          detalle: `Histórico=${formatMoney(histP)} • Actual=${formatMoney(actP)} • ${(pct>=0?"+":"")}${pct.toFixed(2)}%`,
        });
      } else if (hist.presAnual !== null && (!infPres || infPres.error || infPres.costoTotal === null)) {
        checks.push({ codigo: "CMP-03", descripcion: "Comparación presupuesto vs año anterior (INFORME)", status: "WARN", detalle: "No se pudo leer 'Costo total' en INFORME para comparar." });
      } else if (hist.presAnual === null && infPres && !infPres.error && infPres.costoTotal !== null) {
        checks.push({ codigo: "CMP-03", descripcion: "Comparación presupuesto vs año anterior (INFORME)", status: "WARN", detalle: "No se pudo leer presupuesto anual en histórico para comparar." });
      }
    }
  }
  ...
  state.modelData = {
    ids: { distrito: rutaParsed.distrito, nombreDistrito: rutaParsed.nombreDistrito || "", zona: zona || "", amieEje: rutaParsed.amieEje, nombreIE: rutaParsed.nombreIE || "" },
    rutas: { total: rutaParsed.rows.length, beneficiariosTotal: rutaParsed.totalBenef, afcList: rutaParsed.afcList },
    informe: infPres && !infPres.error ? { costoTotal: infPres.costoTotal, costoUnit: infPres.costoUnit, dias: infPres.diasLaborables } : null,
