...
function buildQuipuxTables(modelData, cumplimiento, hasMemo){
  const distrito = modelData.ids.distrito;
  const amie_eje = modelData.ids.amieEje;

  // Cuadro 3. Revisión y documentación (una fila)
  const rev_doc_rows = [{
    distrito,
    amie_eje,
    informe: hasMemo ? "SI" : "NO",
    listado: (modelData.beneficiarios.totalPlanificado>0 && modelData.validacionModelamiento.reglaDvsFGH.ok) ? "SI" : "NO"
  }];

  // Cuadro 4. Validación (una fila por AFC)
  const criteriosTxt = cumplimiento
    ? `${cumplimiento.etiqueta}${cumplimiento.detalle ? " ("+cumplimiento.detalle+")" : ""}${cumplimiento.justificacion ? " Justificación: "+cumplimiento.justificacion : ""}`
    : "SIN CRUCE DAEI";

  const val_rows = modelData.fusiones.afcList.map(afc => ({
    distrito,
    amie_eje,
    amie_afc: afc,
    beneficiarios: modelData.beneficiarios.totalPlanificado,
    criterios: criteriosTxt,
    validacion: modelData.validacionModelamiento.estadoGlobal
  }));
...
function fillCuadrosWorkbook(cwb, modelData, cumplimiento){
  ...
  const {rev_doc_rows, val_rows} = buildQuipuxTables(modelData, cumplimiento, true);
