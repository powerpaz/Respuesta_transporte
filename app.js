...
const modelBuf = await readAsArrayBuffer(state.modelXlsx);
  ...
  if(state.refXlsx && isFilled(amieEje)){
    ...
    if(parsed.error){
      ...
      if(!rng){
        ...
        checks.push({codigo:"RNG-02", descripcion:"Beneficiarios del modelamiento dentro del rango DAEI (Mínimo_F–Máximo_F)", status: ok?"OK":"WARN", detalle:`AMIE ${amieEje} • Total(modelamiento): ${br.total.toFixed(0)} • Rango DAEI: ${rng.min}–${rng.max}` + (ok ? "" : " • Advertencia: no coincide con el número de beneficiarios.")});
      }
    }
  } else {
  ...
  // ========= 10) Cuadros Quipux para heredar tablas en el Word =========
  state.cuadrosData = null;
  if(state.cuadrosXlsx){
    const cwb = XLSX.read(await readAsArrayBuffer(state.cuadrosXlsx), {type:"array"});
    const c = parseCuadrosQuipux(cwb);
    if(c.error){
      checks.push({codigo:"CQX-02", descripcion:"Cuadros (Quipux): estructura válida (Cuadro 1 y Cuadro 2)", status:"WARN", detalle: c.error});
    } else {
      state.cuadrosData = c;
      checks.push({codigo:"CQX-02", descripcion:"Cuadros (Quipux): tablas listas para integrar en la respuesta", status:"OK", detalle:`Cuadro 1 filas: ${c.rev_doc_rows.length} • Cuadro 2 filas: ${c.val_rows.length}`});
    }
  }
  ...
  state.validationMeta = {
    amieEje: amieEje || "",
    rutas_total: br.rows || 0,
    beneficiarios_total: br.total || 0
  };
