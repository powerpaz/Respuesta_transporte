...
function bindDrop(boxId, inputId, metaId, onFile){
  ...
  // Si algún ID cambió o el HTML no cargó bien, no bloqueamos toda la app.
  if(!box || !input || !meta){
    console.error("bindDrop: elemento no encontrado", { boxId, inputId, metaId });
    return;
  }
...
if(builtModel.error){
  ...
  if(state.baseHistXlsx){
    try{
      const bbuf = await readAsArrayBuffer(state.baseHistXlsx);
  ...
  checks.push({
    codigo:"MDL-01",
    descripcion:"ModelData: extracción estructurada (Distrito/AMIE/Beneficiarios/Fusiones)",
    status:"OK",
    detalle:`Distrito ${state.modelData.ids.distrito} • AMIE EJE ${state.modelData.ids.amieEje} • Beneficiarios ${state.modelData.beneficiarios.totalPlanificado.toFixed(0)} • AFC ${state.modelData.fusiones.totalAfc}`
  });

  // --- Comparativo vs base histórica (si existe) ---
  if(state.baseHistXlsx){
    if(state.comparativo){
      const comp = state.comparativo;
      if(comp.tieneHistorico){
        checks.push({codigo:"CMP-01", descripcion:"Comparación con histórico (año anterior)", status:"OK", detalle: buildComparativoTexto(comp)});
        const st = comp.nuevaRuta ? "WARN" : "OK";
        const det = comp.nuevaRuta
          ? `Posible nueva ruta: rutas actual ${comp.actual.rutas} vs histórico ${comp.anterior.rutas}.`
          : `Rutas sin incremento: histórico ${comp.anterior.rutas}, actual ${comp.actual.rutas}.`;
        checks.push({codigo:"CMP-02", descripcion:"Estado de rutas vs año anterior", status: st, detalle: det});
      } else {
        checks.push({codigo:"CMP-01", descripcion:"Comparación con histórico (año anterior)", status:"WARN", detalle:"No se encontró registro equivalente en la base histórica para este Distrito/AMIE."});
      }
    } else {
      checks.push({codigo:"CMP-01", descripcion:"Comparación con histórico (año anterior)", status:"WARN", detalle:"Base histórica cargada, pero no se pudo construir comparativo (revisa columnas/hoja)."});
    }
  }
}
...
function resetAll(){
  ...
  $("modelFile").value="";
  $("refFile").value="";
  const baseFileEl = $("baseFile");
  if(baseFileEl) baseFileEl.value = "";
  $("cuadrosFile").value="";
  $("tplFile").value="";
  ...
  const baseMetaEl = $("baseMeta");
  if(baseMetaEl) setMeta(baseMetaEl, null);
  const cuadrosMetaEl = $("cuadrosMeta");
  if(cuadrosMetaEl) setMeta(cuadrosMetaEl, null);
...
// Inicializa cuando el DOM esté listo (evita que un cache/render raro deje la UI sin listeners)
